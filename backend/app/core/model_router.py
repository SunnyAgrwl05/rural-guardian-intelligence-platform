...
logger = logging.getLogger("rural_guardian.model")
...
class DemoModel:
    async def complete(self, route, text: str) -> str:
        ...
        if route.domain == "compliance":
            return "🛡️ COMPLIANCE SAFETY\n\nPotential voucher/license misuse detected. Do not share or resell codes. Verify eligibility and terms through the official program, document the non-sensitive evidence, and escalate through authorized support if needed."
        if route.domain == "business":
            return "🏪 RURAL BUSINESS GUIDANCE\n\nClarify the product/service, target customers, operating costs and local demand. Use verified local market data before making financial decisions."
        return f"I routed your request to {route.agent}. Configure an authorized model provider for live model-generated responses."
...
class OpenRouterModel:
    def __init__(self):
        ...
        self.client = AsyncOpenAI(api_key=settings.openrouter_api_key, base_url=settings.openrouter_base_url.rstrip("/"), default_headers=self._headers(), timeout=45.0, max_retries=2)
...
class FoundryModel:
    ...
    async def complete(self, route, text: str) -> str:
        def call():
            return self.client.responses.create(input=[
                {"role": "system", "content": [{"type": "input_text", "text": PROMPTS.get(route.domain, PROMPTS["general"])}]},
...
class AzureOpenAIModel:
    def __init__(self):
        ...
        self.client = AsyncOpenAI(api_key=settings.azure_openai_api_key, base_url=settings.azure_openai_endpoint.rstrip("/") + "/openai/v1/", timeout=45.0, max_retries=2)

    async def complete(self, route, text: str) -> str:
        response = await self.client.responses.create(model=settings.azure_openai_deployment, instructions=PROMPTS.get(route.domain, PROMPTS["general"]), input=text)
        return response.output_text
...
def build_model():
    ...
    try:
        ...
        logger.exception("Model provider initialization failed; using demo fallback")
        return DemoModel()
    return DemoModel()
